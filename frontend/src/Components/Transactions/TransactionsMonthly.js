...
import { deleteTransaction, getAllTransactions, getMonthlyAllTransactions, getMonthlyExpenseTransactions, getMonthlyIncomeTransactions } from '../../apiService/transactionsApi';
...
const TransactionsMonthly = () => {
    ...
    const [monthlyAllTransactions, setMonthlyAllTransactions] = useState([])
    ...
    const getData = useCallback(async () => {
        ...
        await getMonthlyExpenseTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setMonthlyExpenseTransactions(response)
            findDiffInTimes(response, "expense")
        }).catch(reason => console.log(reason))
        await getMonthlyIncomeTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setMonthlyIncomeTransactions(response)
            findDiffInTimes(response, "income")
        }).catch(reason => console.log(reason))
        await getMonthlyAllTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setMonthlyAllTransactions(response)
    ...
    const onDeleteClick = async (id) => {
        const allTransactionsCopy = monthlyAllTransactions.filter((transaction) => transaction._id !== id)
                ...
                <div className="leftSide">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {monthlyAllTransactions.map((all, id) => (
                                    <tr key={id}>
                                        <td>{all.title}</td>
                ...
                <div className="allTransactionsExpense">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {monthlyExpenseTransactions.map((expense, id) => (
                                    <tr key={id}>
                                        <td>{expense.title}</td>
                ...
                <div className="allTransactionsIncome">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {monthlyIncomeTransactions.map((income, id) => (
                                    <tr key={id}>
                                        <td>{income.title}</td>
