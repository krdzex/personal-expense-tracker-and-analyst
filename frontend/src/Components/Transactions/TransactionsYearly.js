...
import { deleteTransaction, getAllTransactions, getYearlyAllTransactions, getYearlyExpenseTransactions, getYearlyIncomeTransactions } from '../../apiService/transactionsApi';
...
const TransactionsYearly = () => {
    ...
    const [yearlyAllTransactions, setYearlyAllTransactions] = useState([])
    ...
    const getData = useCallback(async () => {
        ...
        await getYearlyExpenseTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setYearlyExpenseTransactions(response)
            findDiffInTimes(response, "expense")
        }).catch(reason => console.log(reason))
        await getYearlyIncomeTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setYearlyIncomeTransactions(response)
            findDiffInTimes(response, "income")
        }).catch(reason => console.log(reason))
        await getYearlyAllTransactions(authHelper.isAuthentcated().user._id).then(response => {
            response = changeCurrency(response)
            setYearlyAllTransactions(response)
    ...
    const findDiffInTimes = (response, type) => {
        for (let i in response) {
            ...
            let timeDifferenceWeeks = timeNow.diff(createdTime, "weeks")
            let timeDifferenceMonths = timeNow.diff(createdTime, "months")
            ...
            if (timeDifferenceMonths >= 1) {
                setTimeDiff(prevState => ({ ...prevState, [type]: [...prevState[type], { time: timeDifferenceMonths, type: "months" }] }))
                continue
            }
            if (timeDifferenceDays >= 7) {
                setTimeDiff(prevState => ({ ...prevState, [type]: [...prevState[type], { time: timeDifferenceWeeks, type: "weeks" }] }))
                continue
            }
        }
    }
    ...
    const onDeleteClick = async (id) => {
        const allTransactionsCopy = yearlyAllTransactions.filter((transaction) => transaction._id !== id)
        let allExpense = 0;
        let allIncome = 0;
                ...
                <div className="leftSide">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {yearlyAllTransactions.map((all, id) => (
                                    <tr key={id}>
                                        <td>{all.title}</td>
                ...
                <div className="allTransactionsExpense">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {yearlyExpenseTransactions.map((expense, id) => (
                                    <tr key={id}>
                                        <td>{expense.title}</td>
                ...
                <div className="allTransactionsIncome">
                    ...
                    <div className="table">
                        <table>
                            ...
                            {loading && (<tbody>
                                {yearlyIncomeTransactions.map((income, id) => (
                                    <tr key={id}>
                                        <td>{income.title}</td>
...
export default TransactionsYearly;